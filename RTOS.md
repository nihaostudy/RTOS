...
​	静态定义的任务栈是数组，正因为是数组，所以就不能被删除了，这个数组在代码编译时由编译器分配好的。动态分配是malloc，那就自然可以free。
...
```
vTaskList：查看当前所有任务的状态信息列表，这个API需要打开相应的宏开关才能使用
```

​	<img src="C:\Users\abc18\AppData\Roaming\Typora\typora-user-images\image-20250513155216226.png" alt="image-20250513155216226" style="zoom:50%;" />

​	<img src="C:\Users\abc18\AppData\Roaming\Typora\typora-user-images\image-20250513160155042.png" alt="image-20250513160155042" style="zoom:50%;" /><img src="C:\Users\abc18\AppData\Roaming\Typora\typora-user-images\image-20250513160223856.png" alt="image-20250513160223856" style="zoom: 50%;" />

对比栈的水位线和栈的容量，去设置栈的大小，一般将栈的容量设置为平时使用的1-2倍。

假如你给一个任务分配了 **200 字节的栈**；在运行过程中，系统发现，这个任务最多用了 **150 字节**；那就说明，**最少的时候还剩 50 字节的栈没有用上**；这时候，栈水位线就是：**50**。可以配合 `uxTaskGetStackHighWaterMark()` 函数来查看单个任务的栈使用情况

##### 整个系统堆空间的占用率

​	 <img src="C:\Users\abc18\AppData\Roaming\Typora\typora-user-images\image-20250513160701444.png" alt="image-20250513160701444" style="zoom: 50%;" />

系统总体堆大小：configTOTAL_HEAP_SIZA    ((size_t) 3072)

如果有命令行交互指令，可以输入直接答应出堆和栈的使用情况，就不用一直打印出来了。（shell命令行交互方式、CLI命令接口）FreeRTOS还没有这种方式。

### 10.一次性任务

​	实际开发中只会运行一次的任务。运行完别人删除或者自杀。删除操作会把该任务的内存释放掉。

​	vTaskDelete(句柄)

​	**什么时候会使用一次性任务？**

​	比如开机自检、开机显示LOGO、播放用户指导视频等。

​	如果一次性任务不自杀，就会产生一级一级的调用现场，如果没有while（1）,就执行return，return之后执行上层代码，但是任务的上级是操作系统的内核，所以任务中不能有return操作，任务函数的外层是系统内核，所以return后可能跳转到一个野指针，系统则崩溃，所以一定需要主动删除任务。

### 11.如何更好的创建任务？

​	<img src="C:\Users\abc18\AppData\Roaming\Typora\typora-user-images\image-20250513224808195.png" alt="image-20250513224808195" style="zoom:50%;" />

​	内存不够，怎么合理创建任务？

​	一般在mian函数中创建一个一次性任务，启动调度器，在一次性任务中对长期运行的正式任务进行统一创建，在删除任务自身。**mian生一、一生三、三生万物。**创建一次性任务时，还需要将一次性任务设置为较高的优先级。